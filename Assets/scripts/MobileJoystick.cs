...
public class MobileJoystick : MonoBehaviour, IPointerUpHandler, IDragHandler, IPointerDownHandler
{
    ...
    [SerializeField]
    private int dragMovmentDistance = 30;
    [SerializeField]
    private int dragOffsetDistance = 90;
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        Debug.Log(offset);
        joystickTransform.anchoredPosition = offset * dragMovmentDistance;
    ...
    private Vector2 CalculateMovementInput(Vector2 offset)
    {
        ...
        float x = Math.Abs(offset.x) > dragTreshold ? offset.x : 0;
        float y = Math.Abs(offset.y) > dragTreshold ? offset.y : 0;
